...
train_label = lb.fit_transform(df_train[target_col])
predict_label = lb.fit_transform(df_predict[target_col])
...
for i in range(loop_count):
    ...
    #    if predict_data_proba_label['useful'][i] == 1:
    ...
    print("-----評価2-----")
    i = 0
    previous_index = pd.Timestamp(2010, 1, 1, 12)
    clf = RandomForestClassifier(n_estimators=100, max_features=5, warm_start=True)
